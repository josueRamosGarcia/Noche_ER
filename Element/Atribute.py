...
class Atribute(MouseInteractionMixin,QGraphicsEllipseItem):

    #Variable para almacenar el elemento copiado
    copied_item = None

    def __init__(self, pos, scene, width=120, height=35):
        self.scene = scene 
        self.id = id(self)
        ...
        # Asignacion de datos
        self.setData(0,"Atribute")          # Tipo de item
        self.setData(1,"Atributo")          # Texto
        self.setData(2,"Simple")            # Tipo de atributo
        self.setData(3, "varchar(100)")     # Tipo de dato en BD
    ...
    ''' Añade estilo al item'''
    def atribute_derivade(self):
        self.text.setHtml(self.data(1))
        self.pen.setStyle(Qt.PenStyle.DashLine)
        self.setPen(self.pen)
        self.scene.update()

    ''' Añade estilo al item'''
    def atribute_simple(self):
        self.text.setHtml(self.data(1))
        self.pen.setStyle(Qt.PenStyle.SolidLine)
        self.setPen(self.pen)
        self.scene.update()

    ''' Añade estilo al texto'''
    def atribute_llave(self):
        self.text.setHtml(f'<u>{self.data(1)}</u>')
        self.setData(2, "Llave") 
        self.scene.update()

    """ Método para copiar el elemento
    Guarda el elemento copiado en la variable de clase copied_item """
    def copy(self):
        Atribute.copied_item = self
